...
matern2D_big <- inla.spde2.pcmatern(mesh,
                                    ...
                                    prior.sigma = c(1, 0.1)) #0.01 at p 0.1 works
...
# saveRDS(m4, file = "Outputs/badgers_lgcp_model_1km/final_badger_vaccination_model.RDS")
...
# saveRDS(lp4, file = "Outputs/badgers_lgcp_model_1km/vac_linear_predictor.RDS")
...
# saveRDS(rp4, file = "Outputs/badgers_lgcp_model_1km/vac_response_predictor.RDS")
...
ggplot() + 
  gg(data = x, aes(fill = sd), geom = "tile") +
  geom_sf(data = ireland_counties, fill = NA) + 
  # geom_sf(data = badger_subset, alpha = 0.5, size = 1) +
